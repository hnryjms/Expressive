...
router.get('/database', function(req, res) {
	var c = require('../package.json').config || {};
...
router.post('/database', function(req, res) {
	...
	req.data.try(config, function(error) {
		if (error) {
			...
			c.config = config;
			fs.writeFile(path.join(__dirname, '../', 'package.json'), JSON.stringify(c, null, '\t'), function(error){
				res.redirect('/install/site');
			});
		}
	});
...
router.post('/site', function(req, res) {
	...
	Option.findOne({ name: 'title' }, function(err, one) {
		...
		one.save(function(err) {
			...
			user.validateUser(req.body['password'], function(error) {
				if (error) {
				...
				user.save(function(err) {
					...
					req.login(user, function(err) {
						...
						var c = require('../package.json');
						c.config.installed = true;
						c.config.options.server.auto_reconnect = false;
						fs.writeFile(path.join(__dirname, '../', 'package.json'), JSON.stringify(c, null, '\t'), function(error){
